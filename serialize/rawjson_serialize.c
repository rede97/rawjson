...
ssize_t inline rawjson_ser_i32(rawjson_ser_t *ser, int32_t number)
{
    char buf[32];
...
ssize_t inline rawjson_ser_u32(rawjson_ser_t *ser, uint32_t number)
{
    char buf[32];
...
ssize_t inline rawjson_ser_i64(rawjson_ser_t *ser, int64_t number)
{
    char buf[32];
...
ssize_t inline rawjson_ser_u64(rawjson_ser_t *ser, uint64_t number)
{
    char buf[32];
...
ssize_t inline rawjson_ser_float(rawjson_ser_t *ser, float number, int ndigit)
{
    return rawjson_ser_double(ser, (double)number, ndigit);
}

ssize_t inline rawjson_ser_true(rawjson_ser_t *ser)
{
    const char s_bool[] = "true";
    return ser->write_cb(ser, s_bool, sizeof(s_bool) - 1);
}

ssize_t inline rawjson_ser_false(rawjson_ser_t *ser)
{
...
ssize_t inline rawjson_ser_array_begin(rawjson_ser_t *ser)
{
    return ser->write_cb(ser, array_start, static_len(array_start));
}

ssize_t inline rawjson_ser_array_end(rawjson_ser_t *ser)
{
    return ser->write_cb(ser, array_end, static_len(array_end));
}

ssize_t inline rawjson_ser_array_split(rawjson_ser_t *ser)
{
